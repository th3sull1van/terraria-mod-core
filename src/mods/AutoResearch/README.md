...
# 🔬 AutoResearch — Automated Journey Mode Research & Sacrifice for Vanilla Terraria
...
- **🛡️ Journey Mode Isolation**:
  - Automatically inert when playing Classic, Mediumcore, or Hardcore characters (`player.difficulty != 3`), leaving vanilla non-Journey gameplay completely untouched.
...
The configuration file is located at `mods/AutoResearch/config.json`:
...
| `IncludeVoidBag` | `bool` | `true` | Scans and auto-researches items inside the player's Void Bag (Void Vault). |
...
| :--- | :--- | :--- | :--- |
| `Terraria.Player` | `GetItem(Item newItem, GetItemSettings settings)` | `Prefix` | Intercepts item acquisition and instantly researches/sacrifices items, reducing stacks or consuming them before inventory placement. |
