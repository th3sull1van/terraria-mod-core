...
- **Configurable Simultaneous Lines**:
  - Overrides vanilla's single-bobber limitation to support up to **4 (or more)** simultaneous functional fishing lines.
...
- **Dual-Layer Multi-Catch Synchronization**:
  ...
  - **Reel-In Multi-Drop Guarantee (`BobberPullPatch`)**: When reeling in (manually or via `AutoFishing`), all floating bobbers in water (`ai[0] == 0f`) roll their fishing loot tables before retraction, catching and retrieving items simultaneously.

- **Legitimate Bait & Tackle Box Rules**:
  - Every caught fish or crate consumes its respective bait item from inventory, fully respecting the Tackle Box chance (`accTackleBox`).

---

## Configuration Reference

Located at `mods/FishingLinePlus/config.json`:
